...
import { getPrograms, addProgram, deleteProgram, getDepartments, updateProgram } from '../../../services/academicService';
...
const Programs = () => {
    ...
    const handleCreateOrUpdate = async (e) => {
        ...
        try {
            if (editingId) {
                ...
                await addProgram(formData.name, formData.departmentId, formData.duration);
                setToast({ message: "Program added successfully", type: "success" });
            }
            handleCloseModal();
            fetchData();
        } catch (error) {
    ...
    const columns = [
        { key: 'name', label: 'Program Name', render: (val) => <span style={{ color: 'white', fontWeight: 500 }}>{val}</span> },
    ...
    return (
        <div>
            <div style={{ marginBottom: '2rem' }}>
                <h1 style={{ fontSize: '1.875rem', fontWeight: 'bold', margin: '0 0 0.5rem 0', color: 'white' }}>Programs</h1>
                <p style={{ color: '#9ca3af', margin: 0 }}>Manage academic programs.</p>
            </div>
            ...
            <SimpleModal isOpen={isModalOpen} onClose={handleCloseModal} title={editingId ? "Edit Program" : "Add Program"}>
                <form onSubmit={handleCreateOrUpdate} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <label style={{ color: '#e5e7eb', fontSize: '0.875rem' }}>Program Name</label>
                        <input
                            value={formData.name}
